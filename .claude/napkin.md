...
|------|--------|-----------------|--------------------|
| 2026-07-31 | self | TaskTree 命令后在 `/workspace/unidesk` 继续读取 Api2Business 源码，导致只读命令找不到文件 | TaskTree 操作和 Api2Business 开发分开调用，并为每次命令显式设置正确工作目录 |
...
| 2026-08-01 | self | 把 Api2Business L1 聚合生命周期误写为不支持的 `native restart` | Api2Business 聚合重载固定使用 `native stop --component all` 后接 `native start --component all`，再查聚合状态 |
...
| 2026-08-02 | tool | Api2Business product-smoke 只等待 `tbody tr`，初始空态占位行使 scores 步骤以 `rowCount=1` 提前通过 | 统一页验收不能只看 rowCount；必须同时断言无 fatal banner、空态消失、数据行标识和已加载状态 |
...
| 2026-08-03 | self | 明知项目规则禁止裸 `tsc`，仍在并行检查中调用了 `bunx tsc --noEmit`，且误用了不存在的 `build` 脚本 | 先读取 `package.json`，只使用项目声明的受控检查入口；Api2Business 使用 `bun run check`，不猜测脚本名 |
| 2026-08-03 | tool | Api2Business `native start/stop --component all --json` 成功执行时仍可能完全无输出 | 无输出一律不作成功证据，立即用 `native status --component all --json` 回读；生命周期 CLI 可见性需独立修复 |
| 2026-08-03 | self | 给批量导入增加专用 timeout 时一度在 `mutate()` 内直接 fetch，绕过了统一认证头注入 | 传输选项必须沿 `request()` 统一路径透传，不能复制请求实现 |
| 2026-08-03 | runtime | Sub2API 原生账号测试 API 返回独立管理测试结果，但不会写入普通 `usage_logs`、`ops_error_logs` 或 failover 链 | Api2Business 必须标注 `admin-account-test-response` 且声明 `ordinaryLogRecorded: false`；在官方测试链复用普通日志前保持自动探活关闭，不伪造评分样本 |
| 2026-08-03 | user | 探活隔离分组若公开或用上游 URL 命名，会泄露供应商信息并允许其他用户选到该组 | 每账号使用 `is_exclusive=true` 的私有组，名称固定为 `api2business-probe-<accountId>`，只绑定目标账号和专用 Key；不得出现上游 URL、域名或费率 |
...
| 2026-08-03 | self | Api2Business `web-probe product-smoke` 误把页面内的 PK01 后端标签当成 Web target，并误用通用 profile | Api2Business Web 使用 owning target `NC01`；上游资产页使用 profile `scores`，PK01 只表示 Sub2API 后端目标 |
| 2026-08-04 | tool | Api2Business 经营页拆出 OAuth 成本、上游页合并到评分页后，UniDesk operations smoke 仍等待旧 DOM 和旧独立页面 | Web 架构调整后同步更新 owning smoke 页面清单与只读请求分类；经营页只验账本、审计和经营摘要，`POST /api/scores/rank` 视为只读计算 |
...
## User Preferences
- 评分页调整记录必须跟随页面选择的自动刷新周期更新；独立“刷新记录”和全局刷新按钮都必须重新请求数据并展示加载或失败状态，不能静默复用缓存。
- 池级综合质量和上游参与比例排除 `monitor-user` 的自动探活流量；账号级评分保留探活样本，用于评价无自然请求账号。
- 账号质量分和等级只由实际错误率、切号率、延迟等质量指标决定；样本数量只作为证据量提示，不得降低评分、等级或改变相同质量事实的判定。
...
## Patterns That Work
- Api2Business L1 运行与验收使用项目 CLI 和 `--over-api`，不裸调运行组件。
...
- 大批量 runtime 删除即使 CLI 因输出预算返回失败，也必须通过 Api2Business 排队数据库回读判断真实终态。
...
- 不在 TaskTree 的 `/workspace/unidesk` 工作目录中顺带执行 Api2Business 源码命令。
