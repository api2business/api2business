...
test("projects YAML-owned account import defaults", () => {
  const service = new AccountImportService({ operations: { accountImportDefaults: {
    priority: 1, capacity: 16, groupIds: [2, 3], sourceProxyId: 3, perAccountProxy: false, freeCostThresholdCny: 1, plusCostThresholdCny: 7,
  } } } as AppConfig, {} as Sub2ApiReadClient);
  expect(service.options().defaults).toEqual({
    priority: 1, capacity: 16, groupIds: [2, 3], sourceProxyId: 3, perAccountProxy: false, freeCostThresholdCny: 1, plusCostThresholdCny: 7, unitCostCny: null, planType: "k12",
  });
  expect(service.options().planTypes).toEqual([{ id: "k12", name: "K12" }, { id: "plus", name: "Plus" }, { id: "free", name: "Free" }]);
});
